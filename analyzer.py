...
class CryptoBiasAnalyzer:
    ...
    def format_signal(self, symbol, final_dir, final_score, conv,
        ...
        mcap   = fund["mcap"]/1e6 if fund and fund.get("mcap") else 0
        is_long = "LONG" in final_dir
        dir_str = "LONG" if is_long else "SHORT"
        regime  = tech.get("regime", {}).get("regime_btc", "?")
        bd = active_bd
        filled  = min(final_score, 12)
        ...
        lines = [
            ...
            f"EMA    : {ema.get('trend','?')}",
            f"Wyckoff: {htf['wyckoff']['phase']}",
            "-" * 32,
            f"Score  : {final_score}/12 [{bar}]",
            f"L1:{bd.get('L1_regime',0)} L2:{bd.get('L2_structure',0)} L3:{bd.get('L3_ema',0)} L4:{bd.get('L4_elliott',0)} L5:{bd.get('L5_ltf',0)}",
            f"L6:{bd.get('L6_fundamental',0)} L7:{bd.get('L7_defi',0)} L8:{bd.get('L8_sentiment',0)}",
            "=" * 32,
        ]
        return "\n".join(lines)
...
"""
    ...
    def scan_market_by_mcap(self, min_mcap: float = None,
            ...
            try:
                ...
                futures_tickers = [
                    {"symbol": s.replace("/USDT","USDT"),
                     "quoteVolume": tickers[s].get("quoteVolume",0)}
        ...
        for sym, vol in candidates[:max_symbols * 3]:
            ...
            if fund and fund.get("mcap", 0) >= min_mcap:
                result.append((sym, fund["mcap"]))
