...
class CryptoBiasAnalyzer:
    ...
    async def analyze(self, symbol: str, precomputed_tech=None,
        ...
        whale_ctx = precomputed_whale or await get_whale_context(symbol)
...
"""
    ...
    async def quick_scan(self, symbol: str) -> tuple:
        ...
        # Gate on HIGH_THRESH — main.py only enqueues HIGH conviction signals.
        # Scoring MOD (6-8) then calling full analyze() wastes 3+ API round trips
        # per symbol. Skip early here instead.
        ...
        report = await self.analyze(
            ...
            precomputed_whale=whale_ctx,   # avoid second get_whale_context() call
        )
