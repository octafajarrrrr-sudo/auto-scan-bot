...
async def _get(url: str, params: dict = None, ttl: int = 900) -> dict | None:
    key = url + str(params)
    now = time.time()
...
async def get_token_price(symbol: str) -> dict:
    ...
    data = await _get(f"{COINS}/prices/current/{coin_id}")
...
# ══════════════════════════════════════════════════════════════════════════════
async def get_chain_tvl(chain: str = "Ethereum") -> dict:
    """TVL chain tertentu — deteksi inflow/outflow modal."""
    data = await _get(f"{BASE}/v2/historicalChainTvl/{chain}")
...
async def get_dex_volume(chain: str = "all") -> dict:
    """Volume DEX — deteksi lonjakan volume mendadak."""
    params = {"excludeTotalDataChart": "true", "excludeTotalDataChartBreakdown": "true"}
    if chain != "all":
        data = await _get(f"{BASE}/overview/dexs/{chain}", params)
    else:
        data = await _get(f"{BASE}/overview/dexs", params)
    if not data:
        return {}
...
async def get_protocol_tvl(protocol_slug: str) -> dict:
    ...
    data = await _get(f"{BASE}/protocol/{slug}")
    if not data:
        return {}
...
async def get_stablecoin_overview() -> dict:
    ...
    data = await _get(f"{STABLES}/stablecoins?includePrices=true")
...
async def get_bridge_volume() -> dict:
    ...
    data = await _get(f"{BASE}/overview/bridges?includeChains=true")
    if not data:
        return {}
    vol24h = data.get("total24h", 0)
    vol7d  = data.get("total7d", 0)
...
# ══════════════════════════════════════════════════════════════════════════════
async def get_derivatives_overview() -> dict:
    """Open Interest agregat — deteksi penumpukan posisi leveraged."""
    data = await _get(f"{BASE}/overview/derivatives")
    if not data:
        return {}
...
async def get_liquidations_summary() -> dict:
    """
    Ringkasan liquidasi via DefiLlama.
    Deteksi cluster liquidasi yang bisa memicu sweep.
    """
    data = await _get(f"{BASE}/liquidations/eth", ttl=300)  # cache 5 menit
    if not data:
...
async def get_token_unlocks(symbol: str) -> dict:
    ...
    data = await _get(f"{BASE}/emission/{slug}", ttl=3600)  # cache 1 jam
    if not data:
        return {"symbol": symbol, "available": False, "unlock_risk": "UNKNOWN"}
...
# ══════════════════════════════════════════════════════════════════════════════
async def get_full_defi_context(symbol: str) -> dict:
    """
    Gabungan semua data DefiLlama untuk 1 koin.
    Digunakan oleh analyzer untuk menambah bobot confluence.
    """
    unlock, dex_vol, deriv, stables, hacks = await asyncio.gather(
        get_token_unlocks(symbol),
        get_dex_volume(),
