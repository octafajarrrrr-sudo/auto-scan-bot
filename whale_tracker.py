...
async def _get(url, params=None, ttl=300):
    key = url + str(params)
    now = time.time()
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params, timeout=TIMEOUT,
                                   headers={"User-Agent": "CryptoBiasBot/2.0"}) as r:
                if r.status == 200:
                    d = await r.json()
...
async def get_taker_whale_signal(symbol: str) -> dict:
    """
...
async def get_oi_price_divergence(symbol: str) -> dict:
    """
    Deteksi divergence antara Open Interest dan Price.
    Classic SMC whale signal.
    """
    oi_data = await _get(f"{FAPI}/openInterestHist",
                   params={"symbol": f"{symbol.upper()}USDT",
                           "period": "4h", "limit": 6})
...
async def get_liquidation_whale(symbol: str) -> dict:
    """
    Cek recent liquidations untuk deteksi stop hunt / whale flush.
    """
    data = await _get(f"{FAPI}/forceOrders",
...
async def get_exchange_flow_signal() -> dict:
    """
    ...
    """
    data = await _get(f"{LLAMA}/overview/bridges", ttl=1800)
    if not data:
        return {"available": False}
...
async def get_whale_context(symbol: str) -> dict:
    """Gabungkan semua whale signals untuk 1 simbol."""
    taker, oi_div, liq, flows = await asyncio.gather(
        get_taker_whale_signal(symbol),
        get_oi_price_divergence(symbol),
        get_liquidation_whale(symbol),
        get_exchange_flow_signal()
    )
    return {
        "symbol":    symbol,
        "taker":     taker,
...
async def score_whale(symbol: str, direction: str, ctx: dict = None) -> tuple[int, list]:
    """
    Whale scoring (0-1).
    ...
    """
    if ctx is None:
        ctx = await get_whale_context(symbol)
