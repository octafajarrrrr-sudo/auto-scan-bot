...
class TechnicalEngine:
    ...
    def full_analysis(self, symbol, htf="1d", ltf="4h") -> dict | None:
        ...
        # Structure weight (CHoCH dicek DULU agar tidak tertangkap "BULLISH"/"BEARISH")
        s_str_htf = htf_structure["structure"]
        if "CHoCH BULLISH" in s_str_htf:
            bullish_signals += 1   # reversal signal, bukan confirmed trend
        elif "CHoCH BEARISH" in s_str_htf:
            bearish_signals += 1
        elif "BULLISH" in s_str_htf:
            bullish_signals += 2   # confirmed HH+HL
        elif "BEARISH" in s_str_htf:
            bearish_signals += 2   # confirmed LH+LL
