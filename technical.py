...
class TechnicalEngine:
    ...
    # ──────────────────────────────────────────────────────────────────────
    def detect_market_regime(self, symbol, regime_tf="1d") -> dict:
        """
        ...
        df_btc_w = self.fetch_ohlcv("BTC", regime_tf, 250)
        # Coin pada regime timeframe
        df_sym_w = self.fetch_ohlcv(symbol, regime_tf, 100)
    ...
    def full_analysis(self, symbol,
        ...
        df_mtf = self.fetch_ohlcv(symbol, mtf,  200)
        df_ltf = self.fetch_ohlcv(symbol, ltf,  300)
        ...
        # ── Market Regime (1D) ─────────────────────────────────────────
        regime = self.detect_market_regime(symbol, regime_tf=regime_tf)
