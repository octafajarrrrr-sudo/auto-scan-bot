"""
MAIN — Entry Point & Orchestrator (PTB v21 compatible)
Menjalankan Telegram Bot dan Scanner/Execution Loop secara concurrent.
"""
import asyncio
import logging
...
# Shared analyzer instances (akan dibersihkan saat shutdown)
_scanner_analyzer = None
_eval_analyzer = None


async def scanner_loop(app):
    """Loop periodik untuk auto-scan market."""
    ...
    while True:
        try:
            ...
            candidates = await _scanner_analyzer.scan_market_by_volume(
                min_vol=5_000_000,  # $5M minimum volume (bisa Anda sesuaikan)
                max_symbols=max_results,
            )

            if not candidates:
                await _tg_send("❌ Tidak ada koin yang lolos volume filter ($5M).")
...
async def daily_reset_loop():
    """Reset equity dan cache setiap hari pukul 00:00 UTC."""
...
async def post_init(app):
    """Dipanggil setelah aplikasi siap, untuk menjadwalkan tugas latar belakang."""
    # Jalankan post_init milik bot.py (mendaftarkan perintah)
    await bot.post_init(app)

    # Buat background tasks
    app.create_task(scanner_loop(app))
    app.create_task(execution_loop(app))
    app.create_task(daily_reset_loop())
    logger.info("Semua background tasks sudah dijadwalkan.")


async def post_shutdown(app):
    """Tutup semua koneksi graceful."""
