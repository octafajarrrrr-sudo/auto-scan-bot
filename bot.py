...
async def cmd_scan(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    coins = flat_list()
    candidates = []   # (score, report, bias, symbol)

    for sym in coins:
        try:
            result = analyzer.quick_scan(sym)
            # quick_scan sekarang return (report, bias, score) atau (None, "SKIP")
            if isinstance(result, tuple) and len(result) == 3:
                report, bias, score = result
            else:
                report, bias = result[0], result[1]
                score = 0
            if report and bias != "SKIP":
                candidates.append((score, report, bias, sym))
            await asyncio.sleep(1)
        except Exception as e:
            logger.warning(f"Scan skip {sym}: {e}")

    # Urutkan by confidence score — ambil TOP 2 saja
    candidates.sort(key=lambda x: x[0], reverse=True)
    top2 = candidates[:2]

    # Log scan run ke journal
    try:
        from journal import log_scan_run
        log_scan_run(len(coins), len(candidates), [c[3] for c in top2])
    except Exception:
        pass
    ...
    await ctx.bot.send_message(
        chat_id=chat_id,
        text=f"🏆 *TOP 2 SINYAL TERKUAT dari {len(coins)} koin:*",
        parse_mode="Markdown"
    )
    for score, report, bias, sym in top2:
        if len(report) > 4000:
            parts = [report[i:i+4000] for i in range(0, len(report), 4000)]
            for p in parts:
    ...
    await ctx.bot.send_message(
        chat_id=chat_id,
        text=f"✅ *Scan Selesai.* {len(candidates)} sinyal ditemukan → menampilkan 2 terkuat.",
        parse_mode="Markdown"
    )
...
async def auto_scan_job(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for sym in coins:
        try:
            ...
            if isinstance(result, tuple) and len(result) == 3:
                ...
                report, bias = result[0], result[1]; score = 0
            if report and bias != "SKIP":
                candidates.append((score, report, bias, sym))
            await asyncio.sleep(1)
        except Exception as e:
    ...
    if not top2:
        await ctx.bot.send_message(chat_id=chat_id, text="😴 *Auto-Scan Selesai.* Market flat — tidak ada setup.", parse_mode="Markdown")
        return

    await ctx.bot.send_message(chat_id=chat_id, text=f"🏆 *AUTO-SCAN — TOP 2 SINYAL:*", parse_mode="Markdown")
    for score, report, bias, sym in top2:
        if len(report) > 4000:
            parts = [report[i:i+4000] for i in range(0, len(report), 4000)]
            for p in parts:
    ...
    await ctx.bot.send_message(chat_id=chat_id, text=f"✅ *Auto-Scan Selesai.* {len(candidates)} sinyal → top 2 ditampilkan.", parse_mode="Markdown")
