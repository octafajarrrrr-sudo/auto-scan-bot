...
async def cmd_closepapr(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        r = close_paper_trade(int(ctx.args[0]), float(ctx.args[1]), "MANUAL")
    ...
    try:
        ...
        # ── Paper manual ─────────────────────────────────────────────────────
            ...
            if not ticker or not tech:
                ...
                tid = open_paper_trade(sym, e["entry_mode"].split()[0],
                    e["entry"], e["sl"], tp["TP1"], tp["TP2"], tp["TP3"],
                    e.get("sl_pct",0), e.get("rr","N/A"), 0)
                await update.message.reply_text(
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "m:main":
        ...
        await edit_or_new(q,
            ...
            f"*Paper Trade*\nOpen:{ps['open']} WR:{ps['winrate']}% Avg:{ps['avg_pnl']:+.2f}%",
            back("m:main"))
    ...
    # ── Actions ────────────────────────────────────────────────────────────────
        ...
        try: analyzer.binance.fetch_ticker("BTC/USDT"); bk = "✅"
        except: bk = "❌"
        try:
            import requests as _r; _r.get("https://api.coingecko.com/api/v3/ping", timeout=4); cg = "✅"
        except: cg = "❌"
        cfg = cfg_load()
        await edit_or_new(q,
        ...
        if not trades:
            ...
            for t in trades:
                icon = "🟢" if t["bias"] == "LONG" else "🔴"
                lines.append(f"{icon} `#{t['id']}` *{t['symbol']}* {t['bias']}\n"
                             f"  Entry:`{t['entry']:.5f}` SL:`{t['sl']:.5f}` TP1:`{t['tp1']:.5f}` RR:{t['rr']}")
        ...
        await edit_or_new(q,
            ...
            f"Avg PnL:{ps['avg_pnl']:+.2f}% Avg R:{ps['avg_r']:+.2f}R",
            back("m:paper"))
...
async def _run_scan(bot, chat_id, full=False):
    ...
    if not market:
        await bot.send_message(chat_id=chat_id, text="❌ Tidak ada koin lolos MCap filter."); return
    ...
    for score, sym, report, direction in top:
        ticker = analyzer.get_binance_ticker(sym)
        tech   = analyzer.tech.full_analysis(sym, htf=cfg["tf_bias"],
                     mtf=cfg["tf_structure"], ltf=cfg["tf_entry"],
                     regime_tf=cfg["tf_regime"])
        ...
        if tech and tech["execution"]["entry_mode"] not in ("NO TRADE", "BLOCKED (GC/DC filter)"):
            e = tech["execution"]; tp = e["tp"]
            try:
                tid = open_paper_trade(sym, direction, e["entry"], e["sl"],
                    tp["TP1"],tp["TP2"],tp["TP3"],e.get("sl_pct",0),e.get("rr","N/A"),score)
                await bot.send_message(chat_id=chat_id,
                    text=f"📝 Paper `#{tid}` — {sym} {direction} @ `{e['entry']:.5f}`\n"
                         f"Tipe: `{e.get('order_type','MARKET')}`",
                    parse_mode="Markdown")
            except Exception:
                pass
...
async def auto_scan_job(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for ev in check_paper_trades(analyzer.binance):
        e = "✅" if "WIN" in ev["hit_level"] else "❌"
        await ctx.bot.send_message(chat_id=chat_id,
            text=f"{e} Paper `#{ev['trade_id']}` {ev['symbol']} → `{ev['hit_level']}` `{ev['pnl_pct']:+.2f}%`",
            parse_mode="Markdown")
    await _run_scan(ctx.bot, chat_id, full=False)
...
if __name__ == "__main__":
    ...
    app.job_queue.run_repeating(auto_scan_job, interval=cfg["scan_interval_h"]*3600, first=30)
    ...
    logger.info(f"Bot v8 | {cfg['mode']} | {cfg['tf_bias']}/{cfg['tf_structure']}/{cfg['tf_entry']} | Score≥{cfg['min_score']}/14")
